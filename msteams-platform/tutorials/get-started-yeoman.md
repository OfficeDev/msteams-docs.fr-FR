...
Dans ce didacticiel, nous allons passer en premier lieu à la création de votre toute première application Microsoft Teams à l’aide du générateur Yeoman Microsoft Teams. Il vous dirige également tout au long du processus de mise à niveau de teams à l’aide du générateur Yeoman. La condition préalable pour commencer avec ce didacticiel est que vous avez un compte Teams qui permet le chargement de version de version [d’application.](~/concepts/build-and-test/prepare-your-o365-tenant.md)
...
Vous avez besoin d’un éditeur de code. La plupart de cette documentation et images font référence à l’utilisation [Visual Studio Code .](https://code.visualstudio.com) Toutefois, n’hésitez pas à utiliser l’éditeur de texte de votre choix.
...
1. Répondez à l’ensemble des questions posées par le générateur.
   ...
   1. Ensuite, le générateur vous demande quels éléments vous souhaitez ajouter à votre projet. Vous pouvez sélectionner un seul ou n’importe quelle combinaison d’éléments. Pour ce didacticiel, sélectionnez *simplement un onglet.*
...
L’ensemble de questions suivant sera basé sur les éléments précédemment sélectionnés. Pour un onglet, vous devez uniquement fournir un nom et éventuellement choisir si vous souhaitez être en mesure d’utiliser cette application en tant que partie Web SharePoint Online. Une fois que vous avez fourni le nom, le générateur génère le projet et installe toutes les dépendances. Cela prendra une minute ou deux.

## <a name="add-some-code-to-your-tab"></a>Ajouter du code à votre onglet

Une fois le générateur terminé, vous pouvez ouvrir la solution dans votre éditeur de code favori. Prenez une ou deux minutes et familiarisez-vous avec la façon dont le code est organisé. Pour plus d’informations, voir la documentation [relative à la structure du](https://github.com/OfficeDev/generator-teams/wiki/Project-Structure) projet.
...
Tout d’abord, vous devez créer le fichier manifeste de l’application Teams, que vous téléchargez/chargez de nouveau dans Microsoft Teams. Cette tâche est effectuée par la tâche `gulp manifest` Gulp. Cela valide le manifeste et crée un fichier zip dans `./package` le répertoire.
...
Revenir au canal *Général* et choisir *+* d’ajouter un nouvel onglet. Vous devriez voir votre onglet dans la liste d’onglets.
...
## <a name="upgrade-microsoft-teams"></a>Mettre à niveau Microsoft Teams

Vous pouvez également mettre à niveau votre version actuelle de Microsoft Teams vers la dernière version à l’aide du générateur Yeoman Microsoft Teams.
...
4. Sélectionnez le générateur de votre choix dans la liste des générateurs.
   > [!NOTE]
   > Utilisez la barre d’espace pour sélectionner ou effacer une version sélectionnée de Teams parmi les options disponibles.
...
**Félicitations ! Vous avez créé et déployé votre première application Microsoft Teams. Vous avez également mis à niveau Microsoft Teams.**
