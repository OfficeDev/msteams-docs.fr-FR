...
keywords: Connecteur Office 365 teams
ms.date: 04/19/2019
...
1. Incluez le kit de développement logiciel (SDK) Microsoft teams JavaScript. Votre code accède ainsi aux API pour effectuer des opérations courantes telles que l’obtention du contexte d’utilisateur/canal/équipe actuel et l’initiation de flux d’authentification. Initialisez le kit de développement `microsoftTeams.initialize()`logiciel en appelant.
...
>[!Note]
>Les paramètres renvoyés par `getSettings` l’appel ici sont différents de ceux de l’appel de cette méthode à partir d’un onglet et diffèrent de ceux décrits [ici](/javascript/api/%40microsoft/teams-js/settings.settings?view=msteams-client-js-latest).

| Paramètre   | Détails |
|-------------|---------|
| `entityId`       | ID d’entité, tel que défini par votre code lors `setSettings()`de l’appel. |
| `configName`  | Nom de la configuration, tel que défini par votre code `setSettings()`lors de l’appel. |
...
> [!NOTE]
> En raison de raisons de compatibilité entre les clients, votre code doit appeler `microsoftTeams.authentication.registerAuthenticationHandlers()` avec les méthodes de rappel URL et réussite/échec avant `authenticate()`d’appeler.

#### <a name="handling-edits"></a>Gestion des modifications

Votre code doit gérer les utilisateurs qui retournent pour modifier une configuration de connecteur existante. Pour ce faire, appelez `microsoftTeams.settings.setSettings()` lors de la configuration initiale avec les paramètres suivants :
...
En règle générale, cet appel est effectué dans le cadre de votre gestionnaire d’événements Save. Ensuite, lorsque le `contentUrl` code ci-dessus est chargé, votre `getSettings()` code doit appeler pour préremplir les paramètres ou les formulaires dans votre interface utilisateur de configuration.
...
- Incluez deux icônes en suivant les instructions des [icônes](~/concepts/build-and-test/apps-package.md#icons).
- Modifiez la `icons` partie du manifeste pour faire référence aux noms de fichier des icônes au lieu des URL.

Le fichier manifest. JSON suivant contient les éléments de base nécessaires au test et à l’envoi de votre application.

> [!NOTE]
> Remplacez `id` et `connectorId` dans l’exemple suivant avec le GUID de votre connecteur.
...
## <a name="testing-your-connector"></a>Test de votre connecteur

Pour tester votre connecteur, téléchargez-le vers une équipe comme vous le feriez avec n’importe quelle autre application. Vous pouvez créer un package. zip à l’aide du fichier manifeste du tableau de bord du développeur de connecteurs (modifié comme indiqué dans la section précédente) et des deux fichiers d’icône.

Une fois que vous avez téléchargé l’application, ouvrez la liste connecteurs à partir de n’importe quel canal. Faites défiler vers le bas pour voir votre application dans la section **chargée** .
...
Vous pouvez maintenant lancer l’expérience de configuration. N’oubliez pas que ce flux se fait entièrement au sein de Microsoft teams en tant qu’expérience hébergée.
