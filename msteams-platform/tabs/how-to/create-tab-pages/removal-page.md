...
Lorsqu'un utilisateur  sélectionne Supprimer du menu déroulant de l'onglet, Teams charge la page facultative (désignée dans votre page de `removeUrl` **configuration)** dans un IFrame. Ici, l'utilisateur se présente avec un bouton chargé avec la fonction qui appelle et active le bouton Supprimer situé en bas de `onClick()` `microsoftTeams.settings.setValidityState(true)` l'IFrame de la page de suppression. 

Après l'exécution du handler de suppression, ou avertit Teams du `removeEvent.notifySuccess()` résultat de la suppression de `removeEvent.notifyFailure()` contenu.
