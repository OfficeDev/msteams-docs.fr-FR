...
keywords: authentification des équipes SSO API d'authentification unique AAD
...
3. Teams demande le jeton d'accès à l'onglet au point de terminaison Azure Active Directory (AAD) pour l'utilisateur actuel.
4. AAD envoie le jeton d'accès aux onglets à l'application Teams.
...
### <a name="1-create-your-aad-application"></a>1. Créez votre application AAD
...
> * Il est important que le nom de domaine de votre application soit le même que le nom de domaine que vous avez enregistré pour votre application AAD.
...
**Pour enregistrer votre application via le portail AAD**

1. Enregistrez une nouvelle application dans le portail d'[inscriptions d'applications AAD](https://go.microsoft.com/fwlink/?linkid=2083908).
...
> [!NOTE]
>
> * ¹ Si votre application AAD est enregistrée dans le même locataire où vous effectuez une demande d'authentification dans Teams, l'utilisateur ne peut pas être invité à donner son consentement et reçoit immédiatement un jeton d'accès. Les utilisateurs ne consentent à ces autorisations que si l'application AAD est enregistrée dans un locataire différent.
> * ² Si le domaine personnalisé n'est pas ajouté à AAD, vous obtenez une erreur indiquant que le nom d'hôte ne doit pas être basé sur un domaine déjà détenu. Pour ajouter un domaine personnalisé à AAD et l'enregistrer, suivez la [procédure Ajouter un nom de domaine personnalisé à AAD](/azure/active-directory/fundamentals/add-custom-domain), puis répétez l'étape 5. Vous pouvez également obtenir cette erreur si vous n'êtes pas connecté avec des informations d'identification d'administrateur dans la location Office 365.
> * Si vous ne recevez pas le nom d'utilisateur principal (UPN) dans le jeton d'accès renvoyé, vous pouvez l'ajouter en tant que [revendication facultative](/azure/active-directory/develop/active-directory-optional-claims) dans AAD.
...
> [!NOTE]
>
>* La ressource d'une application AAD est généralement la racine de l'URL de son site et l'ID d'application (par exemple, `api://subdomain.example.com/00000000-0000-0000-0000-000000000000`). Cette valeur est également utilisée pour garantir que votre demande provient du même domaine. Assurez-vous que le `contentURL` pour votre onglet utilise les mêmes domaines que votre propriété de ressource.
...
1. Le jeton récupéré à l'aide `getAuthToken()` doit être échangé côté serveur à l'aide du [flux On-Behalf-Of](/azure/active-directory/develop/v2-oauth2-on-behalf-of-flow) ADD de pour accéder à ces autres API Graph. Assurez-vous d'utiliser le point de terminaison v2 Graph pour cet échange.
2. Si l'échange échoue, AAD renvoie une exception d'attribution non valide. Il y a généralement l'un des deux messages d'erreur, `invalid_grant` ou `interaction_required`.
...
4. Lorsque vous demandez plus de consentement à AAD, vous devez inclure `prompt=consent` dans votre [query-string-parameter](~/tabs/how-to/authentication/auth-silent-aad.md#get-the-user-context) à AAD, sinon AAD ne demande pas les autres étendues.
...
La solution d'authentification décrite ci-dessus ne fonctionne que pour les applications et les services qui prennent en charge AAD en tant que fournisseur d'identité. Les applications qui souhaitent s'authentifier à l'aide de services non basés sur AAD doivent continuer à utiliser le [flux d'authentification Web](~/concepts/authentication.md) basé sur les fenêtres contextuelles .

> [!NOTE]
> L'authentification unique est prise en charge pour les applications appartenant au client au sein des locataires AAD B2C.
