...
3. Teams demande le jeton d’accès à l’onglet Azure Active Directory point de terminaison (AAD) pour l’utilisateur actuel.
...
Une autre approche pour obtenir Graph étendues consiste à présenter une boîte de dialogue de consentement à l’aide de notre approche d’authentification Azure AD basée sur le [web existante.](~/tabs/how-to/authentication/auth-tab-aad.md#navigate-to-the-authorization-page-from-your-popup-page) Cette approche implique d’ouvrir une boîte de dialogue de consentement Azure AD.

**Pour demander un consentement supplémentaire à l’aide de l’API Auth**

1. Le jeton récupéré à l’aide doit être échangé côté serveur à l’aide du flux AAD de la part de pour accéder à ces autres API `getAuthToken()` Graph client. [](/azure/active-directory/develop/v2-oauth2-on-behalf-of-flow) Veillez à utiliser le point de terminaison Graph v2 pour cet échange.
...
4. Lorsque vous demandez plus de consentement à AAD, vous devez inclure dans votre paramètre de chaîne de requête à AAD, sinon AAD ne demande pas les `prompt=consent` autres étendues. [](~/tabs/how-to/authentication/auth-silent-aad.md#get-the-user-context)
