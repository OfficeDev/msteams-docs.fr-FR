...
> [!div class="checklist"]
> * Inscrire votre client M365 dans *des Office 365 ciblées*
...
* (Facultatif) [Teams Shared Computer Toolkit](https://aka.ms/teams-toolkit) d’extension Visual Studio Code pour vous aider à mettre à jour votre code
...
Vous devez utiliser le schéma de manifeste d’aperçu [du développeur Teams](/microsoftteams/platform/resources/schema/manifest-schema-dev-preview) et la version du manifeste pour permettre à votre onglet personnel Teams de s’exécuter dans Office et `m365DevPreview` Outlook.
...
> [!IMPORTANT]
> Le code *.html* fichiers de mise à niveau n’est pas pris en charge par les outils de mise à niveau et nécessite des modifications manuelles.
...
[Comme dans Microsoft Teams,](/microsoftteams/platform/tabs/what-are-tabs)les applications d’onglet sont hébergées dans ([éléments iframe](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe)) dans Office et Outlook clients web.

Si votre application [](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy) utilise des en-têtes de stratégie de sécurité du contenu (CSP), assurez-vous que vous autorisez tous les ancêtres d’images suivants dans vos [en-têtes](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/frame-ancestors) CSP :
...
Dans la section **Applications clientes autorisées,** assurez-vous que toutes les valeurs suivantes `Client Id` sont ajoutées :
...
1. Connectez-vous Teams avec votre compte client en bac à sable et assurez-vous que vous êtes sur la prévisualisation du développeur public. Vous pouvez vérifier que vous êtes sur l’aperçu dans le client Teams en cliquant sur le  menu des ellipses (**...**) par votre profil utilisateur et en ouvrant Sur le point de vérifier que l’option  d’aperçu développeur est surliée.
...
Vous pouvez épingler l’application pour un accès rapide, ou vous pouvez trouver votre application dans les ellipses (**...**) volant parmi les applications récentes dans la barre latérale sur la gauche.
