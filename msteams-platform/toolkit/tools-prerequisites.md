...
author: zyxiaoyuer
description: Dans ce module, découvrez les conditions préalables requises pour les outils et le KIT de développement logiciel (SDK)
...
* [Conditions requises de base pour créer votre application Teams](#basic-requirements-to-build-your-teams-app)
...
## <a name="basic-requirements-to-build-your-teams-app"></a>Conditions requises de base pour créer votre application Teams

Vérifiez que les conditions suivantes sont remplies avant de commencer à générer votre application Teams :

| &nbsp; | Conditions requises de base | Pour utiliser| Pour le type d’environnement|
   | --- | --- | --- |
   | **Obligatoire** | &nbsp; | &nbsp; | &nbsp; |
   ...
   | &nbsp; | [Microsoft Teams](https://www.microsoft.com/microsoft-teams/download-app) | Collaborez avec toutes les personnes avec lesquelles vous travaillez via des applications pour la conversation, les réunions, les appels, le tout au même endroit.| JavaScript et SPFx|
   | &nbsp; | [Node.js](https://nodejs.org/en/download/) | Environnement runtime JavaScript principal. Utilisez la dernière version v16 LTS.| JavaScript et SPFx|
   | &nbsp; |[Node Package Manager (NPM)](https://www.npmjs.com/package/@microsoft/teamsfx) | Installez et gérez les packages à utiliser dans les applications Node.js et ASP.NET Core.| JavaScript et SPFx|
   ...
   > * Ajoutez un signet à l’Explorateur Microsoft Graph pour en savoir plus sur les services Microsoft Graph. Cet outil basé sur un navigateur vous permet d’interroger Microsoft Graph et d’y accéder en dehors d’une application.

## <a name="accounts-to-build-your-teams-app"></a>Comptes pour créer votre application Teams

Avant de commencer à générer votre application Teams, vérifiez que vous disposez des comptes suivants :

| Comptes | Pour utiliser| Pour le type d’environnement|
| --- | --- |
...
|[Compte Azure](#azure-account)|Ressources principales sur Azure.| JavaScript et SPFx|
...
* **Compte professionnel Microsoft pour les entreprises**
  ...
  :::row:::
    ...
    :::column span="3":::
       ...
       Le compte fournit l’accès à tous les services cloud Microsoft de niveau entreprise, de petite taille et d’entreprise. Les services incluent Azure, Microsoft Intune ou Microsoft 365. 
...
Vous avez besoin d’un compte Azure pour héberger une application Teams ou les ressources back-end de votre application Teams à l’aide du Kit de ressources Teams dans Visual Studio Code. Vous devez avoir besoin d’un abonnement Azure dans les scénarios suivants :

* Si vous disposez déjà d’une application sur un autre fournisseur de cloud qu’Azure et que vous souhaitez intégrer l’application sur la plateforme Teams, vous devez disposer d’un abonnement Azure.
...
Lors de la création d’une application Teams à l’aide de l’environnement SPFx, vous aurez besoin d’un compte d’administrateur de site de collection SharePoint au moment du déploiement pour l’hébergement. Si vous utilisez un locataire du programme de développement Microsoft 365, vous pouvez utiliser le compte d’administrateur que vous avez créé à ce moment-là.
...
1. Sélectionnez l’icône Teams Toolkit :::image type="icon" source="../assets/images/teams-toolkit-v2/teams-toolkit-sidebar-icon.PNG"::: dans la barre d’outils Teams Toolkit.
...
Si vous ne parvenez pas à afficher l’option **Charger une application personnalisée,** cela indique que vous ne disposez pas de l’autorisation requise pour le chargement indépendant.
...
Si vous disposez de droits d’administrateur, procédez comme suit pour charger l’application personnalisée à l’aide du centre d’administration :
  ...
  1. Sélectionnez **Global (valeur par défaut à l’échelle de l’organisation)**

     :::image type="content" source="../assets/images/teams-toolkit-v2/select-manage-policies.png" alt-text="Sélectionnez Gérer les stratégies":::
  ...
  5. Sélectionnez **Enregistrer**.
     ...
     Vérifiez que vous disposez maintenant de l’autorisation de chargement indépendant en suivant les étapes mentionnées dans [Vérifier l’autorisation de chargement indépendant à l’aide de Visual Studio Code ou du client Teams.](#sideloading-permission)
