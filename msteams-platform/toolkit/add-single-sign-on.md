...
1. Ouvrez `package.json` le fichier et assurez-vous que la version du Kit de développement logiciel (SDK) teamsfx >= 1.2.0
1. Exécutez la `npm install isomorphic-fetch --save` commande dans le `bot` dossier.
1. Pour le script ts, exécutez la commande dans `bot` le `npm install copyfiles --save-dev` dossier et remplacez les lignes suivantes dans `package.json`:
...
2. Identité de l’utilisateur dans Node.js environnement : `OnBehalfOfUserCredential` utilise le flux On-Behalf-Of et le jeton d’authentification unique.
