...
Le Teams Shared Computer Toolkit peut créer des applications React qui seront hébergées dans Azure ou des composants Web SPFx qui seront hébergés sur votre environnement SharePoint M365.  Pour créer une application React à héberger sur Azure :
...
1. (Facultatif) À **l’étape Ressources cloud,** sélectionnez les ressources cloud que votre application utilisera.  Vous pouvez sélectionner l’accès CRUD (créer, lire, mettre à jour, supprimer) à une table SQL une API :
...
Votre application Teams est créée en quelques secondes.  L’application échafaudée contient du code pour gérer l' sign-on unique Azure Active Directory et l’accès au microsoft Graph.  Si vous avez sélectionné des ressources Azure, le code de ces ressources sera également disponible.
