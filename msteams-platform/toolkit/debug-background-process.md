...
* Paquets NPM, applicables à l'application d'onglet, à l'application de robot, à l'application d'extension de messagerie et aux fonctions Azure. Si NPM est installé, le Kit de ressources Teams installe tous les packages NPM
...
* Ports disponibles, si les ports tab, bot, extension de messagerie et Azure Functions ne sont pas disponibles, le débogage local se termine
...
1. [inscrit et configure le bot](#registers-and-configures-bot): Teams Toolkit inscrit et configure votre bot pour l’application d’extension de messagerie ou d’onglet
...
1. Expose une API.
    ...
    a. Configure l'URI d'identification de l'application. Pour l’onglet, `api://localhost/{appId}`. Pour l’extension de bot ou de messagerie,  `api://botid-{botid}`
