...
author: zyxiaoyuer
description: Dans ce module, découvrez comment publier des applications Teams à l’aide du Kit de ressources Teams et comment publier sur une étendue individuelle ou comment charger une version test de l’autorisation
...
* [Publier dans le magasin Microsoft Teams](#publish-to-microsoft-teams-store)
...
* Vérifiez que vous avez suivi l’ensemble des instructions de la publication de votre application dans le magasin Microsoft Teams pour publier votre application.
...
Vous pouvez ajouter une application personnalisée à Teams en chargeant un [package d’application](../concepts/build-and-test/apps-package.md) dans un fichier *.zip directement dans une équipe ou dans un contexte personnel. L’ajout d’une application personnalisée en chargeant un package d’application est appelé chargement indépendant. Il vous permet de tester l’application lors du chargement dans Teams. Vous pouvez créer et tester l’application dans les scénarios suivants :
...
### <a name="to-build-your-app-to-zip-app-package-file"></a>Pour générer votre application pour compresser le fichier de package d’application

Vous devez d’abord exécuter `Provision in the cloud` avant de générer le package d’application. Les étapes suivantes vous aident à générer le package d’application.

* Sélectionnez **Package de métadonnées Zip Teams** sous **DÉPLOIEMENT**.<br>
...
1. Dans le client Teams, sélectionnez **Applications** > **Gérer vos applications** > **chargez une application**.
   ...
   **La fenêtre Charger une application s’affiche** .
...
2. Sélectionnez **Charger une application personnalisée**.
   ...
   Maintenant, l’application est chargée de manière indépendante dans le client Teams et vous pouvez l’ajouter et l’afficher.

## <a name="publish-to-your-organization"></a>Publier pour votre organisation

Lorsque l’application est prête à être utilisée en production, vous pouvez l’envoyer à l’aide de l’API de soumission d’application Teams, appelée à partir de API Graph. L’API de soumission d’application Teams est un environnement de développement intégré (IDE) tel que Microsoft Visual Studio Code installé avec le kit de ressources Teams. Les étapes suivantes vous aident à publier l’application dans votre organisation :

* [Publier à partir du Kit de ressources Teams](#publish-from-teams-toolkit)
* [Approuver sur Administration Center](#approve-on-admin-center)

### <a name="publish-from-teams-toolkit"></a>Publier à partir du Kit de ressources Teams
...
1. Vous pouvez publier votre application Teams de l’une des manières suivantes :
     ...
     * Entrez le déclencheur **Teams : Publier dans Teams à** partir de la palette de commandes.

   :::image type="content" source="../assets/images/teams-toolkit-v2/teams toolkit fundamentals/select-publish.png" alt-text="Sélectionnez Publier.":::

1. Sélectionnez **Installer pour votre organisation**.
   ...
   Maintenant, l’application est correctement publiée sur le portail d’administration et vous voyez l’avis suivant :
...
Le kit de ressources Teams pour Visual Studio Code basé sur l’API de soumission d’application Teams vous permet d’automatiser le processus de soumission à approbation pour les applications personnalisées sur Teams.
  ...
  >
  > * Consultez l’état et les propriétés au niveau de l’organisation des applications.
  > * Approuvez ou chargez de nouvelles applications personnalisées dans l’App Store de votre organisation.
  > * Bloquer ou autoriser les applications au niveau de l’organisation.
  > * Ajouter des applications à Teams.
  ...
  > * [Gérer les paramètres de l’application à l’échelle de l’organisation](https://admin.teams.microsoft.com/policies/manage-apps).
...
1. Sélectionnez l’icône :::image type="icon" source="../assets/images/teams-toolkit-v2/teams toolkit fundamentals/Showall.PNG"::: > **l’application** >  Teams **Gérer les applications**.
   ...
   Dans le widget d’approbation en attente en haut de la page, vous savez quand une application personnalisée est soumise pour approbation. Dans le tableau, une application nouvellement envoyée publie automatiquement l’état des applications envoyées et bloquées. Vous pouvez trier la colonne d’état de publication dans l’ordre décroissant pour rechercher l’application.
...
1. Sélectionnez le nom de l’application pour accéder à la page des détails de l’application. Sous l’onglet **À propos** de, vous pouvez afficher des détails sur l’application, notamment la description, l’état et l’ID de l’application.
...
## <a name="publish-to-microsoft-teams-store"></a>Publier dans le magasin Microsoft Teams
...
Pour plus d’informations, consultez [Publier votre application dans le magasin Microsoft Teams](../concepts/deploy-and-publish/appsource/publish.md#publish-your-app-to-the-microsoft-teams-store).
