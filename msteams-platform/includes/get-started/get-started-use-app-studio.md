...
L’exemple est fourni avec son propre manifeste prédéfini, et est conçu pour créer un package d’application lors de la création du projet. Sur .NET, cette opération est exécutée dans Visual Studio, ainsi que sur le nœud JS `gulp` pour cela, en tapant à la ligne de commande dans le répertoire racine du projet.
...
Votre ID d’application doit ressembler à `2322041b-72bf-459d-b107-f4f335bc35bd`ceci :.
...
Ajoutez un nom de bot tel `Contoso bot`que, puis cliquez sur les deux boutons sous *étendue*.
...
Mettez à jour l’adresse du `https://yourteamsapp.ngrok.io/api/messages`point de `yourteamsapp.ngrok.io` *terminaison du bot* vers, où doit être remplacée par l’URL que vous avez utilisée ci-dessus lors de l’hébergement de votre application.
