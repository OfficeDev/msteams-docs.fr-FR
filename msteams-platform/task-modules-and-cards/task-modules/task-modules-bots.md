...
* Un nouveau type de message `task/fetch``invoke` d’appel : l’utilisation de [l’action](~/task-modules-and-cards/cards/cards-actions.md#action-type-invoke) de carte pour les cartes Bot Framework ou `Action.Submit` de l’action de carte pour les cartes adaptatives, [](~/task-modules-and-cards/cards/cards-actions.md#adaptive-cards-actions) `task/fetch`avec un module de tâche, une URL ou une carte adaptative, est récupérée dynamiquement à partir de votre bot.
* URL de lien profond : à l’aide de la [syntaxe](~/task-modules-and-cards/task-modules/invoking-task-modules.md#task-module-deep-link-syntax) de lien profond pour les modules de tâche, `openUrl` vous pouvez utiliser l’action de carte pour les cartes Bot Framework [](~/task-modules-and-cards/cards/cards-actions.md#action-type-openurl) `Action.OpenUrl` ou l’action de carte pour les cartes adaptatives, respectivement.[](~/task-modules-and-cards/cards/cards-actions.md#adaptive-cards-actions) Avec les URL de lien profond, l’URL du module de tâche ou le corps de la carte adaptative est déjà connu pour éviter un aller-retour serveur par rapport à `task/fetch`.

> [!IMPORTANT]
> Chacun d’eux `url` `fallbackUrl` doit implémenter le protocole de chiffrement HTTPS.

La section suivante fournit des détails sur l’appel d’un module de tâche à l’aide.`task/fetch`

## <a name="invoke-a-task-module-using-taskfetch"></a>Appeler un module de tâche à l’aide de la tâche/extraction

Lorsque l’objet `value` de l’action `Action.Submit` `invoke` de carte ou est initialisé et lorsqu’un utilisateur sélectionne le bouton, `invoke` un message est envoyé au bot. Dans la réponse HTTP `invoke` au message, il existe un objet [TaskInfo](~/task-modules-and-cards/task-modules/invoking-task-modules.md#the-taskinfo-object) incorporé dans un objet wrapper, que Teams utilise pour afficher le module de tâche.
...
1. Cette image montre une carte Hero Bot Framework avec une action **acheter** `invoke` [une carte](~/task-modules-and-cards/cards/cards-actions.md#action-type-invoke). La valeur de la `type` propriété est `task/fetch` et le reste de l’objet `value` peut être de votre choix.
1. Le bot reçoit le `invoke` message HTTP POST.
1. Le bot crée un objet de réponse et le renvoie dans le corps de la réponse POST avec un code de réponse HTTP 200. Pour plus d’informations sur le schéma des réponses, consultez [la discussion sur la tâche/l’envoyer](#the-flexibility-of-tasksubmit). Le code suivant fournit un exemple de corps de la réponse HTTP qui contient un objet [TaskInfo](~/task-modules-and-cards/task-modules/invoking-task-modules.md#the-taskinfo-object) incorporé dans un objet wrapper :
    ...
    L’événement `task/fetch` et sa réponse pour les bots sont similaires à la `microsoftTeams.tasks.startTask()` fonction dans le SDK client.
...
* HTML ou JavaScript `TaskInfo.url`c’est-à-dire : une fois que vous avez validé ce que l’utilisateur a entré, `microsoftTeams.tasks.submitTask()` vous appelez la fonction SDK `submitTask()` appelée ci-après comme à des fins de lisibilité. Vous pouvez appeler `submitTask()` sans paramètre si vous souhaitez Teams le module de tâche, mais vous devez transmettre un objet ou une chaîne à votre `submitHandler`. Passez-le en tant que premier paramètre, `result`. Teams appelle `submitHandler`, `err` est `null`et est `result` l’objet ou la chaîne que vous avez transmis à `submitTask()`. Si vous appelez `submitTask()` avec un `result` paramètre, vous devez transmettre un `appId` ou plusieurs tableaux de chaînes `appId` . Cela permet Teams vérifier que l’application qui envoie le résultat est la même que celle qui a appelé le module de tâche. Votre bot reçoit un `task/submit` message incluant `result`. Pour plus d’informations, [voir charge utile des `task/fetch` `task/submit` messages](#payload-of-taskfetch-and-tasksubmit-messages).
* Carte adaptative : `TaskInfo.card`le corps de la carte adaptative tel qu’il est rempli par l’utilisateur est envoyé au bot `task/submit` par le biais d’un message lorsque l’utilisateur sélectionne un `Action.Submit` bouton.
...
Lorsque l’utilisateur termine avec un module de tâche appelé à partir d’un bot, le bot reçoit toujours un `task/submit invoke` message. Plusieurs options s’offrent à vous pour répondre au `task/submit` message comme suit :
...
| <pre>{<br/>  "task": {<br/>    "type": "message",<br/>    "value": "Message text"<br/>  }<br/>}</pre> | Teams affiche la valeur d’un `value` message dans une boîte de message. |
...
Cette section définit le schéma de ce que votre bot reçoit lorsqu’il reçoit `task/fetch` un objet `task/submit` Bot Framework `Activity` . Le tableau suivant fournit les propriétés de la charge utile et des `task/fetch` `task/submit` messages :

| Propriété | Description                          |
| -------- | ------------------------------------ |
| `type`   | Est toujours `invoke`.           |
| `name`   | Est soit `task/fetch` ou `task/submit`. |
| `value`  | Est la charge utile définie par le développeur. La structure de l’objet `value` est identique à ce qui est envoyé à partir Teams. Dans ce cas, toutefois, il est différent. Elle nécessite la prise en charge de l’extraction dynamique `task/fetch` à partir de Bot Framework, `value` `Action.Submit` c’est-à-dire les actions de carte adaptative, c’est-à-dire.`data` Un moyen de communiquer Teams `context` au bot est nécessaire en plus de ce qui est inclus dans ou `value` `data`.<br/><br/>Combinez « value » et « data » dans un objet parent :<br/><br/><pre>{<br/>  "context": {<br/>    "theme": "default" &vert; "dark" &vert; "contrast",<br/>  },<br/>  "data": [value field from Bot Framework card] &vert; [data field from Adaptive Card] <br/>}</pre>  |

La section suivante fournit un exemple de réception et de réponse à `task/fetch` des `task/submit` messages dans Node.js.
...
Le schéma des actions de carte Bot Framework est différent `Action.Submit` des actions de carte adaptative et la façon d’appeler des modules de tâche est également différente. L’objet `data` dans `Action.Submit` contient un objet `msteams` afin qu’il n’interfère pas avec les autres propriétés de la carte. Le tableau suivant présente un exemple de chaque action de carte :
...
Suivez le [guide pas à pas](../../sbs-botbuilder-taskmodule.yml) pour créer et récupérer le bot de module de tâche dans Teams.
