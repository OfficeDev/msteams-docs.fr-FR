...
description: Dans cet article, découvrez comment créer des liens profonds et les parcourir dans vos applications Microsoft Teams avec des onglets.
ms.topic: how-to
...
* `useMeetNow`: cela inclut un paramètre booléen qui peut être vrai ou faux.
  ...
  * **False** : la valeur `UseMeetNow` par défaut est false, ce qui signifie que lorsqu’un lien profond est partagé pour être mis en scène et qu’il n’y a pas de réunion en cours, une fenêtre contextuelle de calendrier s’affiche. Toutefois, vous pouvez partager directement pendant une réunion.
