...
Récupérer des modèles d'interface utilisateur pour votre conception d'application à partir du Kit d'interface utilisateur Microsoft Teams, qui inclut également des informations complètes sur l'utilisation, l'anatomie, l'accessibilité et les meilleures pratiques.
...
Vous pouvez concevoir des flux de signature d'application pour différents contextes teams et fournisseurs d'identité. L'exemple suivant inclut l'authentification unique (SSO), que nous recommandons pour l'expérience d'authentification la plus simple.
