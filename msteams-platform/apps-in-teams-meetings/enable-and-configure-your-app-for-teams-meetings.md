...
* Savoir comment développer des applications Teams. Pour plus d’informations sur le développement d’Teams application, consultez [Teams développement d’applications](../overview.md).
...
### <a name="context-property"></a>Propriété contextuel
...
| **meetingChatTab** | Onglet dans l’en-tête d’une conversation de groupe entre un ensemble d’utilisateurs pour une réunion planifiée. Vous pouvez spécifier **meetingChatTab** ou **meetingDetailsTab** pour vous assurer que les applications fonctionnent sur mobile. |
| **meetingDetailsTab** | Onglet dans l’en-tête de la vue détails de la réunion du calendrier. Vous pouvez spécifier **meetingChatTab** ou **meetingDetailsTab** pour vous assurer que les applications fonctionnent sur mobile. |
| **meetingSidePanel** | Un panneau en réunion s’est ouvert via la barre unifiée (barre U). |
| **meetingStage** | Une application de l’application `meetingSidePanel` peut être partagée jusqu’à la phase de réunion. Vous ne pouvez pas utiliser cette application sur les clients mobiles ou de salle Teams. |
...
Teams réunions offrent une expérience collaborative à votre organisation. Configurez votre application pour différents scénarios de réunion et pour améliorer l’expérience de réunion. Vous pouvez maintenant identifier les actions qui peuvent être effectuées dans les scénarios de réunion suivants :
...
Vous `meetingSidePanel` pouvez ainsi personnaliser les expériences d’une réunion qui permettent aux organisateurs et aux présentateurs d’avoir différents ensembles de vues et d’actions. Dans le manifeste de votre application, vous devez l’ajouter `meetingSidePanel` au tableau de contextes. Dans la réunion et dans tous les scénarios, l’application est affichée dans un onglet de la réunion de 320 pixels de largeur. Pour plus d’informations, consultez [l’interface FrameContext](/javascript/api/@microsoft/teams-js/microsoftteams.framecontext?view=msteams-client-js-latest&preserve-view=true).
...
La notification en réunion ne doit pas utiliser le module de tâche. Le module de tâche n’est pas appelé dans une conversation de réunion. Une URL de ressource externe est utilisée pour afficher la notification en réunion. Vous pouvez utiliser la `submitTask` méthode pour envoyer des données dans une conversation de réunion.
...
* [Partagez l’intégralité de l’application pour effectuer une mise en scène](#share-entire-app-to-stage) à l’aide du bouton Partager pour mettre en scène dans Teams client.
...
Les participants peuvent partager l’ensemble de l’application à la phase de réunion collaborative à l’aide du bouton Partager pour mettre en scène à partir du panneau côté application.
...
Pour plus d’informations, consultez le [manifeste de l’application](../resources/schema/manifest-schema-dev-preview.md#configurabletabs).

##### <a name="share-specific-parts-of-the-app-to-stage"></a>Partager des parties spécifiques de l’application à mettre en scène

Les participants peuvent partager des parties spécifiques de l’application à la phase de réunion collaborative en utilisant le partage pour mettre en scène les API. Les API sont disponibles dans le SDK client Teams et sont appelées à partir du panneau côté application.

<img src="../assets/images/apps-in-meetings/share-specific-content-to-stage.png" alt="Share specific parts of the app" width = "900"/>

Pour partager des parties spécifiques de l’application à mettre en scène, vous devez appeler les API associées dans la bibliothèque du SDK client Teams. Pour plus d’informations, consultez la [référence de l’API](API-references.md).
...
|Panneau côté réunion | Exemple d’application pour montrer comment ajouter un ordre du jour dans un panneau côté réunion | [View](https://github.com/OfficeDev/Microsoft-Teams-Samples/tree/main/samples/meetings-sidepanel/csharp) |-|
...
* Suivez le [guide pas à pas](../sbs-meetings-sidepanel.yml) pour générer un sidepanel de réunion dans votre réunion Teams.
* Suivez le [guide pas à pas](../sbs-meetings-stage-view.yml) pour partager l’affichage de la phase de réunion dans votre réunion Teams.
...
* [Teams flux d’authentification pour les onglets](../tabs/how-to/authentication/auth-flow-tab.md)
* [Instructions de conception de l’expérience de phase de réunion partagée](~/apps-in-teams-meetings/design/designing-apps-in-meetings.md)
