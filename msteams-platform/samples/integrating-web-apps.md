...
Les API et structures de données de votre application doivent prendre en charge l’application lors de l’intégration à Teams. Pour étendre la prise en charge, vous devez enrichir les API et les structures de données avec des informations contextuelles sur les Teams [](../concepts/build-and-test/deep-links.md)pour le mappage d’identité[, la](../concepts/authentication/configure-identity-provider.md) prise en charge des liens profonds et l’incorporation de [Microsoft Graph](/graph/teams-concept-overview).
