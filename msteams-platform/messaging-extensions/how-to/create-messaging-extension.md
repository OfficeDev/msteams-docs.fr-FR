...
---
# <a name="create-a-messaging-extension-in-microsoft-teams"></a>Créer une extension de messagerie dans Microsoft teams
...
3. Enregistrer votre service Web à l’aide de l’infrastructure de robot
4. Créer votre package d’application
5. Télécharger votre package vers Microsoft teams
...
## <a name="create-your-web-service"></a>Créer votre service Web
...
* Créez votre service Web de a à z. Vous pouvez choisir d’ajouter le kit de développement logiciel (SDK) de robot pour votre langue, ou vous pouvez utiliser directement les charges utiles JSON.

## <a name="register-your-web-service-with-the-bot-framework"></a>Enregistrer votre service Web à l’aide de l’infrastructure de robot

Les extensions de messagerie tirent parti du schéma de messagerie et du protocole de communication sécurisé de l’infrastructure bot. Si vous n’en avez pas encore, vous devez enregistrer votre service Web sur l’infrastructure de robot. ID de l’application Microsoft (il s’agit de l’ID de votre robot de l’intérieur de teams, pour l’identifier à partir d’autres ID d’application que vous pouvez utiliser) et le point de terminaison de messagerie que votre inscription à l’infrastructure de robot sera utilisé dans votre extension de messagerie pour recevoir et répondre à la demande uests. Si vous utilisez une inscription existante, veillez à [activer le canal Microsoft teams](/azure/bot-service/bot-service-manage-channels.md?view=azure-bot-service-4.0).
...
1. Dans le client Teams, ouvrez l’application Studio à partir du menu débord **...** sur le rail de navigation de gauche. S’il n’est pas déjà installé, vous pouvez le faire en le recherchant.
...
3. Ajoutez les détails de votre application (voir [définition de schéma de manifeste](~/resources/schema/manifest-schema.md) pour obtenir les descriptions complètes de chaque champ).
...
6. Si nécessaire, mettez à jour votre adresse de point de terminaison de robot pour qu’elle pointe vers votre bot. Il doit ressembler `https://someplace.com/api/messages`à ce qui suit.
...
Pour ajouter une extension de messagerie, incluez une nouvelle structure JSON de niveau supérieur dans le manifeste de `composeExtensions` votre application avec la propriété. Vous créez une extension de messagerie unique pour votre application, avec un maximum de 10 commandes.
...
L’exemple ci-dessous est un simple objet d’extension de messagerie dans le manifeste de l’application avec une commande de recherche. Il ne s’agit pas du fichier manifeste de l’application, mais uniquement de la partie spécifique aux extensions de messagerie. Consultez le [schéma de manifeste d’application](~/resources/schema/manifest-schema.md) pour obtenir un exemple complet.
...
* [Lien unfurling](~/messaging-extensions/how-to/link-unfurling.md)
