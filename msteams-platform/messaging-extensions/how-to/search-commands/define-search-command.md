...
Pour ajouter la commande de recherche au manifeste de votre application, `composeExtension` vous devez ajouter un nouvel objet au niveau supérieur du manifeste JSON de votre application. Vous pouvez ajouter la commande de recherche à l’aide d’App Studio ou manuellement.
...
Pour ajouter manuellement votre commande de recherche d’extension de messagerie au manifeste de votre application, vous devez ajouter les paramètres suivants `composeExtension.commands` à votre tableau d’objets :
...
La section suivante est un exemple du manifeste d’application simple de l’objet `composeExtensions` définissant une commande de recherche :
...
Suivez le [guide pas à pas](../../../sbs-messagingextension-searchcommand.yml) pour créer une extension de messagerie basée sur la recherche.
