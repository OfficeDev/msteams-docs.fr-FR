...
Les bots sont créés à l’aide de Bot Framework. Si les bots reçoivent une activité de message, le turn handler reçoit une notification de cette activité entrante. Le handler turn envoie ensuite l’activité entrante au handler `OnMessageActivityAsync` d’activité. Dans Teams, cette fonctionnalité reste la même. Si le bot reçoit une activité de mise à jour de conversation, le turn handler reçoit une notification de cette activité entrante et envoie l’activité entrante à `OnConversationUpdateActivityAsync`. Le Teams d’activité vérifie d’abord s’il Teams événements spécifiques. Si aucun événement n’est trouvé, il les transmet ensuite au programme de gestion d’activité de Bot Framework.
...
Les bots sont créés à l’aide de Bot Framework. Si les bots reçoivent une activité de message, le turn handler reçoit une notification de cette activité entrante. Le handler turn envoie ensuite l’activité entrante au handler `onMessage` d’activité. Dans Teams, cette fonctionnalité reste la même. Si le bot reçoit une activité de mise à jour de conversation, le turn handler reçoit une notification de cette activité entrante et envoie l’activité entrante à `dispatchConversationUpdateActivity`. Le Teams d’activité vérifie d’abord s’il Teams événements spécifiques. Si aucun événement n’est trouvé, il les transmet ensuite au programme de gestion d’activité de Bot Framework.
...
Les bots sont créés à l’aide de Bot Framework. Si les bots reçoivent une activité de message, le turn handler reçoit une notification de cette activité entrante. Le handler turn envoie ensuite l’activité entrante au handler `on_message_activity` d’activité. Dans Teams, cette fonctionnalité reste la même. Si le bot reçoit une activité de mise à jour de conversation, le turn handler reçoit une notification de cette activité entrante et envoie l’activité entrante à `on_conversation_update_activity`. Le Teams d’activité vérifie d’abord s’il Teams événements spécifiques. Si aucun événement n’est trouvé, il les transmet ensuite au programme de gestion d’activité de Bot Framework.
...
La liste des handlers définis dans inclut `ActivityHandler` les suivants :
...
Il `TeamsActivityHandler` étend la liste des handlers dans la section principale des handlers Bot Framework pour inclure les éléments suivants :

| Événement | Handler | Description |
| :-- | :-- | :-- |
| channelCreated | `OnTeamsChannelCreatedAsync` | Cette méthode peut être overridée pour gérer un canal Teams en cours de création. Pour plus d’informations, [voir canal créé dans les événements](https://aka.ms/azure-bot-subscribe-to-conversation-events#channel-created) de [mise à jour de conversation](https://aka.ms/azure-bot-subscribe-to-conversation-events). |
...
La liste des Teams d’activité appelés `OnInvokeActivityAsync` à partir du Teams’activité sont les suivants :
...
| actionableMessage/executeAction | `OnTeamsO365ConnectorCardActionAsync` | Cette méthode est invoquée lorsqu’une activité d’action Office 365 de carte de connecteur est reçue du connecteur. |
...
La liste des Teams d’activité appelés `onInvokeActivity` à partir du Teams’activité sont les suivants :
...
| actionableMessage/executeAction | `handleTeamsO365ConnectorCardAction` | Cette méthode est invoquée lorsqu’une activité d’action Office 365 de carte de connecteur est reçue du connecteur. |
...
Il `TeamsActivityHandler` étend la liste des handlers de la section principale des handlers Bot Framework pour inclure les éléments suivants :
...
La liste des Teams d’activité appelés `on_invoke_activity` à partir du Teams’activité sont les suivants :
...
| actionableMessage/executeAction | `on_teams_o365_connector_card_action` | Cette méthode est invoquée lorsqu’une activité d’action Office 365 de carte de connecteur est reçue du connecteur. |
