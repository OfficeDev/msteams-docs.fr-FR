...
Après avoir ajouté le canal Teams à votre chatbot, un **ID** d'application est généré dans la boîte de dialogue. L'ID d'application est un identificateur Microsoft unique généré pour votre bot. Enregistrez l'ID d'application pour créer un package d'application pour Teams.
...
App Studio est une application Teams. Installez App Studio à partir du magasin Teams qui simplifie le processus de création et d'inscription du bot dans Teams : 

   1. Sélectionnez l'icône du Magasin d'applications à partir de l'instance Teams, puis **recherchez App Studio.**
...
1. **Créer le manifeste de l'application Teams dans App Studio**  
...
L'image suivante vous guide pour créer une application dans App Studio :  
...
L'image suivante vous guide pour ajouter les détails de l'application :  
...
1. **Configurer votre bot** Pour configurer le bot, effectuez les étapes suivantes : 
   ...
   L'image suivante vous guide pour configurer un bot :    

   ![Mise en place d'un bot](../../assets/images/get-started/bot-set-up.png) 

   L'image suivante vous guide pour configurer un bot existant :      
...
Pour ajouter votre ID d'application, effectuez les étapes suivantes :  
    1. Sélectionnez **Se connecter à un autre ID de bot** et collez l'ID d'application que vous avez copié précédemment.  
